...
mod tests {
	...
	fn max_digest_interval_works() {
		assert_eq!(config(0, 0).max_digest_interval::<u64>(), 1);
		assert_eq!(config(2, 2).max_digest_interval::<u64>(), 4);
		assert_eq!(config(8, 4).max_digest_interval::<u64>(), 4096);
		assert_eq!(config(::std::u32::MAX, 1024).max_digest_interval::<u64>(), ::std::u32::MAX);
	}
}
