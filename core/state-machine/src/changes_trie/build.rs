...
pub fn prepare_input<'a, B, S, H, Number>(
	...
	where
		...
		Number: BlockNumber,
{
	let config = match changes.changes_trie_config.as_ref() {
		Some(config) => config,
		None => return Ok(None),
	};
...
mod test {
	...
	fn build_changes_trie_nodes_on_non_digest_block() {
		...
		let changes_trie_nodes = prepare_input(&backend, &storage, &changes, &AnchorBlockId { hash: Default::default(), number: 4 }).unwrap();
	...
	fn build_changes_trie_nodes_on_digest_block_l1() {
		let (backend, storage, changes) = prepare_for_build();
		let changes_trie_nodes = prepare_input(&backend, &storage, &changes, &AnchorBlockId { hash: Default::default(), number: 3 }).unwrap();
/*
thread 'changes_trie::build::test::build_changes_trie_nodes_on_digest_block_l1' panicked at 'assertion failed: `(left == right)`
  left: `Some([
	  ExtrinsicIndex(ExtrinsicIndex { block: 4, key: [100] }, [0, 2, 3]),
	  ExtrinsicIndex(ExtrinsicIndex { block: 4, key: [101] }, [1]),
	  ExtrinsicIndex(ExtrinsicIndex { block: 4, key: [103] }, [0, 1]),
	  DigestIndex(DigestIndex { block: 4, key: [100] }, [3]), DigestIndex(DigestIndex { block: 4, key: [102] }, [2]), DigestIndex(DigestIndex { block: 4, key: [105] }, [3])])`,
 right: `Some([
	 ExtrinsicIndex(ExtrinsicIndex { block: 4, key: [100] }, [0, 2, 3]),
	 ExtrinsicIndex(ExtrinsicIndex { block: 4, key: [101] }, [1]),
	 ExtrinsicIndex(ExtrinsicIndex { block: 4, key: [103] }, [0, 1]),
	 DigestIndex(DigestIndex { block: 4, key: [100] }, [1, 3]), DigestIndex(DigestIndex { block: 4, key: [101] }, [1]), DigestIndex(DigestIndex { block: 4, key: [102] }, [2]), DigestIndex(DigestIndex { block: 4, key: [105] }, [1, 3])])`', core/state-machine/src/changes_trie/build.rs:241:3
...
*/
	...
	fn build_changes_trie_nodes_on_digest_block_l2() {
		let (backend, storage, changes) = prepare_for_build();
		let changes_trie_nodes = prepare_input(&backend, &storage, &changes, &AnchorBlockId { hash: Default::default(), number: 15 }).unwrap();
		assert_eq!(changes_trie_nodes, Some(vec![
